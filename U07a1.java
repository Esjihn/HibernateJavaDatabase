        ...
        coursesComboBox.setOnAction(e -> {
            if(totalCredit < MAX_CREDIT_LOAD)
            {
                ...
                if(Boolean.TRUE)
                {
                    ...
                    totalCredit += choice.getCreditHours();
                    creditHoursLabel.setText(Integer.toString(totalCredit));
                    confirmPromptLabel.setText("Registration confirmed for\n course " + choice.getCourseCode());
 
                    // TODO make sure this populates to the label
                    for(CourseRegistration cr :service.getAllCourseRegistrations(learnerIDField.getText()))
                    {
                        registeredCoursesLabel.setText(cr.getLearnerID() + ": " + cr.getCourseCode());
                    }

                    }
                    catch(Exception ex)
                    {
                        log.log(Level.SEVERE, "Entity Manager failed to create Course Registration");
                    }
                }
                else
                {
                    confirmPromptLabel.setText("**Invalid**\nYou have already \nregistered for " + choice.getCourseCode());
                }
            }
            else
            {
                  confirmPromptLabel.setText("**Invalid**\nYou cannot register for \nmore than 9 credits.");
                  // Unit 7 log reminder that TextField will be changed.
                  log.log(Level.INFO, "Clearing name TextField and resetting buffer of all fields to null");
                  // Unit 7 clearing name TextField
                  learnerIDField.clear();
                  learnerIDField.setText(null);
                  registeredCoursesLabel.setText(null);
                  creditHoursLabel.setText(null);  
            }
